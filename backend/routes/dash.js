...
// Ip & Log
router.get('/ip',verifyToken,requireRole('ฝ่ายบุคลากร'),async (req,res) => {
    try{
        const [rows] = await db.query(`select * from tb_log order by id_log desc`)
        res.json(rows)
    }catch(err){
        console.error("Error Get Log",err)
        res.status(500).json({message:'Error Get Log'})
    }
})
